...
class RestResponseBuilderSpec extends FunSpec {
  ...
  def withStringResponse(s: String)(f: Return[BasicHttpResponse] => Any ) = {
    withResponse{ resp =>
      val entity = new StringEntity(s, "UTF-8")
      resp.map(_.setEntity(entity))
      f(resp)
    }
  }

  def withByteArrayResponse(s: Array[Byte])(f: Return[BasicHttpResponse] => Any) = {
    withResponse{ resp =>
